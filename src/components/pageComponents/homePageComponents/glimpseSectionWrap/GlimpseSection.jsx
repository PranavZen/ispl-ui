import React from "react";
import "../glimpseSectionWrap/glimpsesection.css";
import SectionTitle from "../../../common/sectiontitletext/SectionTitle";

function GlimpseSection() {
  return (
    <section id="animeGallery">
      <SectionTitle titleText="Glimpse of ISPL T-10 Season 1" />
      <section id="glimpseSection">
        <div id="image-container">
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3778199/pexels-photo-3778199.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3178767/pexels-photo-3178767.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1553783/pexels-photo-1553783.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/432059/pexels-photo-432059.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1183266/pexels-photo-1183266.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2908889/pexels-photo-2908889.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3014808/pexels-photo-3014808.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2382254/pexels-photo-2382254.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/9255736/pexels-photo-9255736.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
        </div>
        <div id="image-container">
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3778199/pexels-photo-3778199.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3178767/pexels-photo-3178767.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1553783/pexels-photo-1553783.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/432059/pexels-photo-432059.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1183266/pexels-photo-1183266.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2908889/pexels-photo-2908889.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3014808/pexels-photo-3014808.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2382254/pexels-photo-2382254.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/9255736/pexels-photo-9255736.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
        </div>
        <div id="image-container">
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3778199/pexels-photo-3778199.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3178767/pexels-photo-3178767.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1553783/pexels-photo-1553783.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/432059/pexels-photo-432059.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1183266/pexels-photo-1183266.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2908889/pexels-photo-2908889.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3014808/pexels-photo-3014808.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2382254/pexels-photo-2382254.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/9255736/pexels-photo-9255736.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
        </div>
        <div id="image-container">
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3778199/pexels-photo-3778199.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3178767/pexels-photo-3178767.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1553783/pexels-photo-1553783.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/432059/pexels-photo-432059.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1183266/pexels-photo-1183266.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2908889/pexels-photo-2908889.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3014808/pexels-photo-3014808.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2382254/pexels-photo-2382254.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/9255736/pexels-photo-9255736.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
        </div>
        <div id="image-container">
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3778199/pexels-photo-3778199.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3178767/pexels-photo-3178767.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1553783/pexels-photo-1553783.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/432059/pexels-photo-432059.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1183266/pexels-photo-1183266.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2908889/pexels-photo-2908889.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3014808/pexels-photo-3014808.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2382254/pexels-photo-2382254.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/9255736/pexels-photo-9255736.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
        </div>
        <div id="image-container">
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1043474/pexels-photo-1043474.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3778199/pexels-photo-3778199.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3178767/pexels-photo-3178767.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1553783/pexels-photo-1553783.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/432059/pexels-photo-432059.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/1183266/pexels-photo-1183266.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2908889/pexels-photo-2908889.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/3014808/pexels-photo-3014808.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/2382254/pexels-photo-2382254.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
          <div className="image">
            <img
              src="https://images.pexels.com/photos/9255736/pexels-photo-9255736.jpeg"
              alt=""
              className="picture img-fluid"
              loading="lazy"
              data-fancybox
            />
          </div>
        </div>
      </section>
    </section>
  );
}

export default GlimpseSection;
